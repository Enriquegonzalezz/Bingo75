...
const PAQUETES = [{ id: 'paquete-original', nombre: 'Paquete Original' }];
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-[#f8df7e] mt-2 font-semibold">
          ...
          <Button
            ...
            onClick={recargar}
            disabled={loading}
            className="bg-[#ffd402] text-[#1d1d1b] hover:bg-[#e6c000]"
          >
        ...
        <div className="bg-[#1d1d1b] rounded-xl shadow-lg p-6 mb-6 border border-[#ffd402]/30">
          <div className="flex items-center gap-3 mb-4">
            <Package className="w-6 h-6 text-[#ffd402]" />
            <h3 className="text-lg font-bold text-[#ffd402]">Seleccionar Grupo de Cartones</h3>
          </div>
                ...
                className={`
                  ...
                  ${
                    paqueteActual === paquete.id
                      ? 'bg-[#ffd402] text-[#1d1d1b] scale-105 shadow-lg'
                      : 'bg-[#124723] text-white hover:bg-[#68b258] hover:scale-102'
                  }
        ...
        <div className="bg-[#1d1d1b] rounded-xl shadow-lg p-6 mb-8 border border-[#ffd402]/30">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-1 w-full">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#ffd402] w-5 h-5" />
              <Input
                type="text"
                value={busqueda}
                onChange={(e) => handleBusqueda(e.target.value)}
                placeholder="Buscar por serial o número de cartón..."
                className="pl-10 bg-[#2d2d2b] border-[#3d3d3b] text-white placeholder-gray-500 focus:border-[#ffd402]"
              />
            </div>
            <div className="text-[#f8df7e] text-sm whitespace-nowrap">
              Mostrando <span className="font-bold text-[#ffd402]">{indiceInicio + 1}</span> -{' '}
              <span className="font-bold text-[#ffd402]">
                {Math.min(indiceFin, cartonesFiltrados.length)}
              </span>{' '}
              de{' '}
              <span className="font-bold text-[#ffd402]">
                {cartonesFiltrados.length.toLocaleString()}
              </span>
            </div>
          </div>
          {busqueda && (
            <p className="text-sm text-[#68b258] mt-2 font-semibold">
        ...
        {loading ? (
          <div className="text-center py-16">
            ...
            <p className="text-[#f8df7e] font-semibold">Cargando cartones...</p>
          </div>
        ) : cartones.length === 0 ? (
          <div className="text-center py-16 bg-[#1d1d1b] rounded-xl shadow-lg border border-[#ffd402]/30">
            ...
            <h3 className="text-xl font-bold text-[#ffd402] mb-2">Error al cargar cartones</h3>
            <p className="text-[#f8df7e] mb-6">No se pudieron cargar los cartones desde el JSON</p>
            <Button onClick={recargar} className="bg-[#ffd402] text-[#1d1d1b] hover:bg-[#e6c000]">
          ...
          <div className="text-center py-16 bg-[#1d1d1b] rounded-xl shadow-lg border border-[#ffd402]/30">
            ...
            <p className="text-[#f8df7e]">No se encontraron cartones con &quot;{busqueda}&quot;</p>
          </div>
        ) : (
          <>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-8">
              {cartonesPaginados.map((carton) => (
                <div
                  key={carton.id}
                  className="bg-white rounded-xl shadow-lg p-4 hover:shadow-xl transition-all hover:scale-[1.02] border border-[#124723]/20 hover:border-[#ffd402]/50"
                >
                  <div className="mb-3 text-center">
                    <p className="text-xs text-[#124723]/70">Cartón</p>
                    <p className="text-xl font-bold text-[#124723]">#{carton.numero_carton}</p>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 bg-[#1d1d1b] rounded-xl p-4 border border-[#ffd402]/30">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2 rounded-lg bg-[#124723] text-white disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[#68b258] transition-colors"
                  ...
                  <div className="flex items-center gap-1">
                    {paginasVisibles[0] > 1 && (
                      <>
                        <button
                          onClick={() => irAPagina(1)}
                          className="w-10 h-10 rounded-lg bg-[#124723] text-white hover:bg-[#68b258] transition-colors font-bold"
                        >
                          1
                        </button>
                        {paginasVisibles[0] > 2 && <span className="text-[#ffd402] px-2">...</span>}
                      </>
                    )}
                      ...
                      <button
                        ...
                        className={`w-10 h-10 rounded-lg font-bold transition-colors ${
                          pagina === paginaActual
                            ...
                            : 'bg-[#124723] text-white hover:bg-[#68b258]'
                        }`}
                      >
                        {pagina}
                      </button>
                    ...
                    {paginasVisibles[paginasVisibles.length - 1] < totalPaginas && (
                      <>
                        {paginasVisibles[paginasVisibles.length - 1] < totalPaginas - 1 && (
                          <span className="text-[#ffd402] px-2">...</span>
                        )}
                        <button
                ...
                <div className="text-[#f8df7e] text-sm">
                  Página <span className="font-bold text-[#ffd402]">{paginaActual}</span> de{' '}
                  <span className="font-bold text-[#ffd402]">{totalPaginas}</span>
                </div>
              </div>
            )}
          </>
