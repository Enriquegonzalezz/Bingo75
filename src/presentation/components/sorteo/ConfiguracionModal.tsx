...
export interface ConfiguracionRonda {
  ...
  nombrePatronPersonalizado?: string;
  cartonesIndividuales?: number[];
}
...
function PatronModalidad({
  ...
  return (
    ...
    >
        ...
        {modalidad.patron.map((fila, i) =>
          fila.map((activo, j) => (
            <div
              ...
              className={`
                w-4 h-4 rounded-sm
                ${i === 2 && j === 2 ? 'bg-white' : activo ? 'bg-[#ffd402]' : 'bg-[#1d1d1b]'}
              `}
            />
          ))
        )}
      ...
      <span
        className={`text-xs font-semibold text-center ${seleccionado ? 'text-black' : 'text-white'}`}
      >
        {modalidad.nombre}
      </span>
    </button>
...
const PAQUETE_ORIGINAL: PaqueteDisponible = {
  id: 'paquete-original',
  nombre: 'Paquete Original (Local)',
  total_cartones: 10002,
  esLocal: true,
};
...
export function ConfiguracionModal({
  ...
  const [rangoHasta, setRangoHasta] = useState(1000);
  ...
  const [inputCartonesIndividuales, setInputCartonesIndividuales] = useState('');
  const [usarCartonesEspecificos, setUsarCartonesEspecificos] = useState(false);
  const [paquetesDisponibles, setPaquetesDisponibles] = useState<PaqueteDisponible[]>([
    PAQUETE_ORIGINAL,
  ]);
  const [loadingPaquetes, setLoadingPaquetes] = useState(true);

  const paqueteActual =
    paquetesDisponibles.find((p) => p.id === paqueteSeleccionado) || PAQUETE_ORIGINAL;
  const totalCartonesPaquete = paqueteActual.total_cartones;
  ...
  useEffect(() => {
    const cargarPaquetes = async () => {
      try {
        ...
        if (response.ok) {
          const paquetesDB = (data.paquetes || []).map((p: any) => ({
            ...p,
            esLocal: false,
          }));
          setPaquetesDisponibles([PAQUETE_ORIGINAL, ...paquetesDB]);
        }
      } catch (error) {
        console.error('Error al cargar paquetes:', error);
        setPaquetesDisponibles([PAQUETE_ORIGINAL]);
      } finally {
  ...
  useEffect(() => {
    const maxCartones = paqueteActual.total_cartones;
    if (rangoHasta > maxCartones) {
      setRangoHasta(maxCartones);
    }
    if (rangoDesde > maxCartones) {
      setRangoDesde(1);
    }
  }, [paqueteSeleccionado, paqueteActual.total_cartones, rangoDesde, rangoHasta]);
  ...
  const patronPersonalizado =
    rondas.find((r) => r.numero === rondaSeleccionada)?.patronPersonalizado || patronVacio;
  const nombrePatronPersonalizado =
    rondas.find((r) => r.numero === rondaSeleccionada)?.nombrePatronPersonalizado || '';
  ...
  const toggleModalidad = (id: string) => {
    setRondas((prev) =>
      prev.map((r) => {
        ...
        const nuevasModalidades = r.modalidades.includes(id) ? [] : [id];
  ...
  const handleConfirmar = () => {
    if (rangoHasta > totalCartonesPaquete) {
      alert(
        `Error: El rango máximo (${rangoHasta}) excede el total de cartones del paquete (${totalCartonesPaquete})`
      );
      return;
    }
  ...
  const setNombrePatronPersonalizadoFn = (nombre: string) => {
    setRondas((prev) =>
      prev.map((r) => {
        if (r.numero !== rondaSeleccionada) return r;
  ...
  const handleNumeroRondasChange = (n: number) => {
    const nuevoNumero = Math.max(1, Math.min(20, n));
    setNumeroRondas(nuevoNumero);
    ...
    if (rondaSeleccionada > nuevoNumero) {
      setRondaSeleccionada(nuevoNumero);
    }
  };

  const togglePavosoRonda = (numeroRonda: number) => {
    setRondas((prev) =>
      prev.map((r) => (r.numero === numeroRonda ? { ...r, pavosoActivo: !r.pavosoActivo } : r))
    );
  };
  ...
  const actualizarPremioRonda = (numeroRonda: number, premio: number) => {
    setRondas((prev) => prev.map((r) => (r.numero === numeroRonda ? { ...r, premio } : r)));
  };

  const actualizarMonedaRonda = (numeroRonda: number, moneda: 'USD' | 'VES') => {
    setRondas((prev) => prev.map((r) => (r.numero === numeroRonda ? { ...r, moneda } : r)));
  };
    ...
    setRondas((prev) =>
      prev.map((r) =>
        r.numero === numeroRonda
          ? { ...r, cartonesIndividuales: cartones.length > 0 ? cartones : undefined }
          : r
      )
    );
  };
  ...
  const agregarCartonesIndividuales = () => {
    const numeros = inputCartonesIndividuales
      ...
      .filter((n) => !isNaN(n) && n >= 1 && n <= totalCartonesPaquete);
  ...
  const eliminarCartonIndividual = (numeroRonda: number, carton: number) => {
    setRondas((prev) =>
      prev.map((r) => {
        ...
        return {
          ...r,
          cartonesIndividuales: nuevosCartones.length > 0 ? nuevosCartones : undefined,
        };
      })
    );
  };
  ...
  const handleRangoDesdeChange = (value: string) => {
    ...
    setRangoDesde(Math.max(1, Math.min(num, totalCartonesPaquete)));
  };

  const handleRangoHastaChange = (value: string) => {
    const num = parseInt(value) || totalCartonesPaquete;
    setRangoHasta(Math.max(rangoDesde, Math.min(num, totalCartonesPaquete)));
  };

  const handlePaqueteChange = (nuevoPaqueteId: string) => {
    setPaqueteSeleccionado(nuevoPaqueteId);
    const nuevoPaquete = paquetesDisponibles.find((p) => p.id === nuevoPaqueteId);
    if (nuevoPaquete) {
      setRangoDesde(1);
      setRangoHasta(Math.min(1000, nuevoPaquete.total_cartones));
    }
  };

  const todasRondasTienenModalidades = rondas.every((r) => r.modalidades.length > 0);
  const rangoValido = rangoHasta <= totalCartonesPaquete && rangoDesde <= rangoHasta;
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
          ...
          <div className="bg-[#fbf7da] rounded-xl p-4 mb-6">
            ...
            <div className="flex items-center gap-4 mt-3 pt-3 border-t border-[#ffd402]/30 flex-wrap">
              ...
              <div className="flex items-center gap-2 bg-[#6a2818] rounded-lg px-3 py-1 h-10">
                ...
                <span className="text-[#fbf7da] font-bold">
                  {rondaActual?.moneda === 'USD' ? '$' : 'Bs.'}
                </span>
                <input
                  type="number"
                  value={rondaActual?.premio || 0}
                  onChange={(e) =>
                    actualizarPremioRonda(rondaSeleccionada, parseFloat(e.target.value) || 0)
                  }
                  min={0}
              ...
              <button
                ...
                className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold transition-all ${
                  rondaActual?.menosAciertosActivo
                    ? 'bg-[#000] text-white'
          ...
          <div className="bg-white rounded-lg p-4 mb-6 min-h-[180px]">
            <p className="text-sm text-gray-600 mb-3">
              Seleccionando figuras para{' '}
              <span className="font-bold text-[#124723]">Ronda {rondaSeleccionada}</span>
            </p>
            ...
            {categoriaActiva === 'PERSONALIZADO' ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-sm text-gray-600 text-center">
                  Haz clic en las celdas para activar/desactivar. El centro (FREE) siempre está
                  activo.
                </p>

                <div className="w-full max-w-xs">
                  <label className="block text-sm font-semibold text-[#124723] mb-1">
                    Nombre del patrón:
                  </label>
                  <input
                    type="text"
                    value={nombrePatronPersonalizado}
                    onChange={(e) => setNombrePatronPersonalizadoFn(e.target.value)}
                ...
                <div className="bg-[#124723] p-4 rounded-xl shadow-lg">
                  <div className="grid grid-cols-5 gap-1 mb-2">
                    {['B', 'I', 'N', 'G', 'O'].map((letra) => (
                      <div
                        key={letra}
                        className="w-12 h-8 flex items-center justify-center text-[#ffd402] font-black text-lg"
                      >
                        {letra}
                      </div>
                    ))}
                      ...
                      fila.map((activo, j) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-12 h-12 rounded-lg font-bold text-sm flex items-center justify-center transition-all duration-200 transform hover:scale-105
                              ${
                                esCentro
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-sm text-gray-600">
                    Celdas activas:{' '}
                    <span className="font-bold text-[#124723]">{celdasActivasPersonalizado}</span>
                  </span>
                  <button
                    onClick={() => {
                ...
                {celdasActivasPersonalizado < 2 && (
                  <p className="text-xs text-red-500">
                    Selecciona al menos 2 celdas para crear un patrón válido
                  </p>
                )}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gradient-to-r from-[#6a2818] to-[#8b3a24] rounded-lg p-4">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border-2 border-[#ffd402] rounded-lg font-bold text-[#1d1d1b] bg-[#fbf7da]"
                  >
                    {loadingPaquetes ? (
                  ...
                  <p className="text-xs text-[#fbf7da] mt-2">
                    {paqueteActual.esLocal
                      ? '📁 Paquete cargado desde archivo local'
                      : '🗄️ Paquete cargado desde base de datos'}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      min={rangoDesde}
                      max={totalCartonesPaquete}
                      className={`w-20 px-2 py-1 border-2 rounded text-center font-bold text-black ${
                        rangoHasta > totalCartonesPaquete
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  <span className="font-bold text-[#124723]">
                    {totalCartonesPaquete.toLocaleString()}
                  </span>
                </div>
                ...
                {!rangoValido && (
                  <div className="flex items-center gap-2 p-3 bg-red-100 border border-red-300 rounded-lg">
                    ...
                    <span className="text-sm text-red-700 font-semibold">
                      El rango excede el total de cartones ({totalCartonesPaquete.toLocaleString()})
                    </span>
                  </div>
                )}
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      setUsarCartonesEspecificos(e.target.checked);
                      if (!e.target.checked) {
                ...
                {usarCartonesEspecificos && (
                  <div className="bg-[#fbf7da] rounded-lg p-4">
                    <h4 className="text-[#000] font-bold mb-2 text-sm">
                      Cartones Específicos para Ronda {rondaSeleccionada}
                    </h4>
                    ...
                    <div className="flex gap-2 mb-3">
                      <input
                        ...
                        placeholder={`Ej: 20, 35, 40 (máx ${totalCartonesPaquete})`}
                        className="flex-1 px-3 py-2 rounded-lg border-2 border-gray-300 text-[#000] font-semibold text-sm bg-white"
                      />
                      <button
                        onClick={agregarCartonesIndividuales}
                        className="px-4 py-2 bg-[#6a2818] text-white font-bold rounded-lg hover:bg-[#8a3828] transition-colors text-sm"
                      >
                        Agregar
                      </button>
                    </div>

                    {rondaActual?.cartonesIndividuales &&
                      rondaActual.cartonesIndividuales.length > 0 && (
                        <div className="bg-white rounded-lg p-3">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-[#6a2818] text-xs font-bold">
                              {rondaActual.cartonesIndividuales.length} cartón(es)
                            </span>
                            <button
                          ...
                          <div className="flex flex-wrap gap-2">
                            {rondaActual.cartonesIndividuales.map((carton) => (
                              <div
                                key={carton}
                                className="bg-[#6a2818] text-white px-3 py-1 rounded-full text-sm font-bold flex items-center gap-2"
                              >
                                #{carton}
                                <button
                                  onClick={() =>
                                    eliminarCartonIndividual(rondaSeleccionada, carton)
                                  }
                                  className="hover:text-red-300 font-black"
                                >
                                  ×
                                </button>
                              </div>
                ...
                <div className="bg-[#f8df7e] rounded p-3 text-center">
                  <p className="text-sm font-semibold text-[#124723]">
                    ...
                    <span className="font-black">
                      {(rangoHasta - rangoDesde + 1).toLocaleString()}
                    </span>{' '}
                    cartones
                  </p>
                </div>
                ...
                {rondas.map((ronda) => (
                  <div
                    ...
                    className={`p-3 rounded-lg border-2 cursor-pointer transition-all ${
                      rondaSeleccionada === ronda.numero ? 'ring-2 ring-[#ffd402]' : ''
                    } ${
                      ronda.modalidades.length === 0
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-xs font-bold ${ronda.modalidades.length === 0 ? 'text-red-500' : 'text-[#68b258]'}`}
                      >
                        {ronda.modalidades.length === 0 ? '⚠️ Sin figura' : '✓ Configurada'}
                      </span>
                    </div>
                    {ronda.modalidades.length > 0 && (
                      <p className="text-xs text-gray-600 mt-1">
                        {modalidades.find((m) => m.id === ronda.modalidades[0])?.nombre ||
                          ronda.modalidades[0]}
                        {ronda.premio > 0 &&
                          ` • ${ronda.moneda === 'USD' ? '$' : 'Bs.'}${ronda.premio}`}
                      </p>
                    )}
                  </div>
                ))}
          ...
          <div className="flex justify-end gap-3 mt-6">
            <button
              ...
              className="px-6 py-3 bg-[#000] text-[#fff] font-bold rounded-lg hover:bg-gray-800 transition-colors"
            >
              Cerrar
            </button>
            <button
              onClick={handleConfirmar}
              disabled={!todasRondasTienenModalidades || !rangoValido}
              className={`px-6 py-3 font-bold rounded-lg flex items-center gap-2 transition-colors
                ${
                  !todasRondasTienenModalidades || !rangoValido
                    ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
