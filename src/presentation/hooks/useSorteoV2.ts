    ...
    try {
      ...
      if (configuracion) {
        ...
        if (rondaConfig?.cartonesIndividuales && rondaConfig.cartonesIndividuales.length > 0) {
          ...
          setCartonesEnJuego(filtrados);
          console.log(
            `📊 Ronda ${rondaActual}: Usando rango ${desde}-${hasta} (${filtrados.length} cartones)`
          );
        }
      } else {
    ...
    (numero: number) => {
      ...
      if (nuevosGanadores.length > 0) {
        ...
        if (menosAciertosActivo && hayGanadoresBingo) {
          ...
          console.log(
            `📊 Menos aciertos calculados: ${menosAciertosFiltered.length} cartones con ${menosAciertosFiltered[0]?.aciertos || 0} aciertos`
          );
        }
      }
    },
